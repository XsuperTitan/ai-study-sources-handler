...
- 千问 VL 图片理解、DeepSeek 摘要/笔记/学习指南、千问图像或万相生成记忆图。
...
$env:QWEN_IMAGE_MODEL='qwen-image-2.0-pro'
$env:QWEN_IMAGE_FREE_QUOTA_ONLY='false'
...
图片生成默认启用千问图像，复用 `DASHSCOPE_API_KEY`，生成 `abstract`、`classic`
和 `whiteboard` 三张记忆图。`QWEN_IMAGE_FREE_QUOTA_ONLY=false` 表示不在本地维护免费
额度计数，额度不足时交给百炼接口返回错误；如需本地保护，可设置为 `true`，并同时配置
`QWEN_IMAGE_FREE_QUOTA_CONFIRMED=true` 和 `QWEN_IMAGE_FREE_QUOTA_REMAINING`。

可选变量包括 `QWEN_IMAGE_BASE_URL`、`QWEN_IMAGE_ENABLED`、`WANX_MODEL`、`YT_DLP_PATH`、
`YT_DLP_COOKIES_FROM_BROWSER`、`APP_STORAGE_ROOT` 和 `CHROMA_BASE_URL`。设置
`QWEN_IMAGE_ENABLED=false` 时，插图生成回退到万相配置。
...
- 千问图像或万相接口返回临时 URL 后会立即下载到本地；首页资料卡片支持
  “抽象记忆图 / 图表记忆 / 白板记忆图”三种记忆图，缺失的历史图片可按需补生成；
  插图失败只会令资料包进入 `PARTIALLY_READY`。
