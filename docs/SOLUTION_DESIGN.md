...
- 使用本地目录和 JSON 保存资料包处理数据，使用 MySQL 保存掌握状态与学习行为事件。
- 使用进程内线程池执行异步任务，暂不引入消息队列。
- 使用统一领域接口隔离本地存储、DeepSeek、千问 VL、万相和 `yt-dlp`。
- 使用千问 Embedding 生成 1024 维向量，由独立 Chroma 进程保存内容块索引。
...
| 存储 | 本地目录和 JSON；MySQL 学习状态与行为事件 |
| 学习概览 | 掌握总数、本周新增、连续天数、七日趋势、近期关键词 |
| 全库 RAG | 原始内容块索引、资料范围筛选、证据引用问答 |
...
- 用户注册、登录和多用户权限。
- 资料包、Job 和内容块的全量 MySQL 迁移；Redis、RabbitMQ、Kafka。
- 对象存储。
- 自动掌握度计算；本阶段仅实现用户主动标记、归档和恢复。
- 多计划并行、自动复习提醒和复杂日历同步。
- 复习任务日历和消息提醒。
- Word 文件解析。
- 视频音频下载和 ASR。
...
| AI 插图数量 | 默认 3 张：`abstract`、`classic`、`whiteboard` |
| 视频链接数量 | 每个视频资料包 1 个 |
| 支持的视频平台 | Bilibili |
| 视频内容来源 | 已有字幕，不下载音视频 |
...
- PRD 中的“学习计划”已落地为单活跃 PLAN 工作台：基于资料包快照生成排期步骤、记录学习过程并支持按需 AI 重排；多计划并行、自动复习提醒和复杂日历同步仍留作后续能力。
- PRD 中的“用户与权限、业务数据全量 MySQL 化”暂不实现；固定本地用户下，学习域先使用 MySQL，
  资料包处理数据继续使用本地 JSON。
...
- 默认通过 DashScope 千问图像接口调用，复用 `DASHSCOPE_API_KEY`。
- `app.qwen-image.enabled=false` 时回退到 DashScope 万相接口。
- 从已生成笔记的高层摘要构建绘图 Prompt，并按 `abstract`、`classic`、`whiteboard`
  三种变体生成。
- 返回图片 URL 后立即下载到本地。
...
```json
{
  ...
  "citationCount": 26,
  "sourceImageAssetIds": [],
  "abstractIllustrationAssetId": null,
  "illustrationAssetId": null,
  "whiteboardIllustrationAssetId": null,
  "model": "deepseek-chat",
...
- 笔记中使用不可见标记 `<!-- asset-slot:illustration -->` 指定记忆图注入位置。
- 不使用外部临时图片 URL。
- AI 补充段落添加“AI 补充”提示。

AI 插图完成后只更新 `note.json` 中的 `abstractIllustrationAssetId`、`illustrationAssetId`
或 `whiteboardIllustrationAssetId`。前端在详情页视觉区提供三图切换，并在旧版正文标记位置
兼容注入可用图片；图片不存在时显示摘要和空状态，不修改模型生成的正文。导出功能后续再将标记物化为相对图片路径。
...
- 笔记标题。
- 核心摘要。
- 最多 8 个一级知识点，或 3-5 个抽象记忆分区。
- 明确的变体风格要求：
  - `abstract`：复古手绘 sketchnote、旧纸张、装饰横幅、分区叙事、箭头/编号/图标、短促中文标签。
  - `classic`：图表式学习信息图。
  - `whiteboard`：白板速记式学习图。
...
2. 按 `IllustrationVariant` 枚举顺序生成 `abstract`、`classic`、`whiteboard`。
3. 默认调用千问图像；禁用千问图像时调用万相。
4. 获取临时图片 URL。
5. 在 URL 有效期内下载。
6. 校验 Content-Type 和非空字节。
7. 保存为 `outputs/abstract-illustration.png`、`outputs/classic-illustration.png`
   或 `outputs/whiteboard-illustration.png` 对应的本地 asset。
8. 更新 note metadata。
...
└── outputs/
    ...
    ├── report.json
    ├── report.md
    ├── abstract-illustration.png
    ├── classic-illustration.png
    └── whiteboard-illustration.png
```
...
```json
{
  ...
  "outputs": {
    "noteReady": false,
    "reportReady": false,
    "abstractIllustrationReady": false,
    "illustrationReady": false,
    "whiteboardIllustrationReady": false
  }
}
```
...
首页资料卡片的 `cover` 字段保留 `imageUrl`，并优先使用 `abstract`，缺失时回退
`classic`。同时返回 `visualVariants.abstract`、`visualVariants.classic` 和
`visualVariants.whiteboard`。每个 variant 包含 `ready`、`generating`、`imageUrl`
和 `errorMessage`，用于首页全局切换“抽象记忆图 / 图表记忆 / 白板记忆图”。历史资料包
缺少某个 variant 时，前端可调用：

```http
POST /api/v1/packages/{packageId}/illustrations/{abstract|classic|whiteboard}/generate
```

接口在图片已存在时直接返回资料包摘要；图片缺失时创建 `ILLUSTRATION` Job 并返回
`202 { packageId, rootJobId }`。传入 `replace=true` 时允许覆盖已有图片并重新生成。
...
| `DASHSCOPE_API_KEY` | 是 | 千问 VL、千问图像与万相密钥 |
| `QWEN_VL_BASE_URL` | 否 | 百炼兼容接口 |
| `QWEN_VL_MODEL` | 是 | 千问 VL 模型 |
| `QWEN_IMAGE_BASE_URL` | 否 | 千问图像接口根地址 |
| `QWEN_IMAGE_MODEL` | 否 | 默认 `qwen-image-2.0-pro` |
| `QWEN_IMAGE_ENABLED` | 否 | 默认 `true`，禁用后回退万相 |
| `QWEN_IMAGE_FREE_QUOTA_ONLY` | 否 | 默认 `false`，不做本地免费额度拦截 |
| `QWEN_IMAGE_FREE_QUOTA_CONFIRMED` | 否 | 仅 `QWEN_IMAGE_FREE_QUOTA_ONLY=true` 时使用 |
| `QWEN_IMAGE_FREE_QUOTA_REMAINING` | 否 | 仅 `QWEN_IMAGE_FREE_QUOTA_ONLY=true` 时使用 |
| `WANX_BASE_URL` | 否 | 万相接口根地址 |
| `WANX_MODEL` | 否 | 万相模型，仅千问图像禁用后需要 |
...
当前已实现基于现有 `ContentBlock` 的首版全库 RAG：

1. 后端按约 1800 字、200 字重叠切分原始内容块。
2. 千问 `text-embedding-v4` 分别以 `document` 和 `query` 类型生成 1024 维向量。
3. Spring AI `ChromaApi` 将向量、原文和来源元数据写入独立 Chroma 进程。
4. 检索支持全库或资料包范围过滤，同一资料包最多保留两个片段。
5. DeepSeek 只能依据检索证据回答，返回的引用仍通过 `sourceRef` 回到原资料。

不直接将最终 Markdown 笔记作为唯一索引，避免模型整理内容覆盖原始证据。Chroma 或
Embedding 失败属于可降级阶段，不阻塞笔记与学习指南。

### 22.6 学习概览

学习概览从 `learning_subject_state` 和 `PACKAGE_MASTERED` 行为事件实时聚合：

- “当前掌握”统计 `mastered = true` 且 `source_deleted_at IS NULL` 的现存资料。
- 已掌握资料删除后保留标题、关键词、掌握时间和删除时间快照，并在“已删除留档”中展示最近 5 条。
- 学习中的资料删除时不创建掌握状态，因此不会进入学习成果留档。
- 本周新增、连续学习天数和七日趋势保留真实学习历史，包括后来被删除的已掌握资料，并按
  `Asia/Shanghai` 自然日计算。
- 近期关键词仅从现存已掌握资料聚合，按最近 30 天词频和最近掌握时间排序，最多返回 10 条。

`GET /api/v1/learning/overview` 的关键字段：

| 字段 | 含义 |
| --- | --- |
| `masteredTotal` | 当前仍存在的已掌握资料数 |
| `deletedMasteredTotal` | 已掌握后被删除的历史留档数 |
| `deletedMastered` | 最近 5 条删除留档，包含原资料 ID、标题、关键词、掌握时间和删除时间 |
| `recentKeywords` | 当前掌握资料产生的近期 Top 10 关键词 |
...
- 决策：资料包、Job、内容块和生成结果继续使用本地 JSON。
- 原因：保留已验证的处理流水线和文件组织方式。
- 代价：资料处理域仍不适合多实例和高并发。
- 缓解：学习域独立通过 Repository 端口接入 MySQL，后续再逐域迁移。

### ADR-001A：学习域使用 MyBatis 与 Flyway

- 决策：使用 MyBatis 原生 Mapper 和 XML SQL，不引入 JPA、Hibernate 或 MyBatis-Plus。
- 原因：当前学习域以批量查询、条件更新、MySQL Upsert 和追加事件为主，显式 SQL 更便于控制幂等与索引。
- 数据表：`learning_subject_state` 保存当前掌握状态，`user_activity_event` 保存不可变行为历史。
- 一致性：掌握状态变更与事件写入使用同一数据库事务；资料删除采用请求、文件删除、结果事件三阶段记录。
- 演进：通过 `subject_type + subject_id` 支持后续知识点、复习任务和测验主体。
...
| V1.4 | 2026-06-22 | 增加 `abstract` 抽象记忆图、三图 Tab、详情页三图切换和千问图像本地额度开关说明 |
